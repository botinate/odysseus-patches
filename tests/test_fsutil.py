...
def test_default_mode_is_owner_only_with_no_world_readable_window(tmp_path):
    # mkstemp creates 0600 from the start, so a secret is never world-readable
    p = tmp_path / "secret"
    atomic_write_text(p, "sk-token", mode=0o600)
    assert p.stat().st_mode & 0o777 == 0o600


@pytest.mark.skipif(os.name != "posix", reason="POSIX file modes")
def test_explicit_mode_applied(tmp_path):
    p = tmp_path / "public"
    atomic_write_text(p, "{}", mode=0o644)
...
def test_symlinked_destination_is_replaced_not_followed(tmp_path):
    ...
    atomic_write_text(dest, "new content", mode=0o600)
